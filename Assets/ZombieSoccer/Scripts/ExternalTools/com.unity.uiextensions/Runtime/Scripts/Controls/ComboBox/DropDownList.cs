...
namespace UnityEngine.UI.Extensions
{
	...
	public class DropDownList : MonoBehaviour
	{
			...
			for (int i = 0; i < _panelItems.Count; i++)
			{
				if (i < Items.Count)
				{
					...
					_panelItems[i].btn.onClick.AddListener(() =>
					{
						...
						ViewPlacementCharacters.PresetIndex.Value = ii;						
					});
				}
		...
		{
			Debug.Log($"item: {indx}, current: {_selectedIndex}");
			if (indx != _selectedIndex) OnSelectionChanged?.Invoke(indx);
        ...
        public void SetPresetIndex(int indx)
        {
            Debug.Log($"item: {indx}, current: {_selectedIndex}");
            if (indx != _selectedIndex) OnSelectionChanged?.Invoke(indx);
		...
		public void ToggleDropdownPanel(bool directClick)
		{
			...
			//Debug.LogError($"directClick: {directClick}");
