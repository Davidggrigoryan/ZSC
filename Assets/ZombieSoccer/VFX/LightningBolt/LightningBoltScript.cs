...
namespace DigitalRuby.LightningBolt
{
    ...
    {
#if UNITY_EDITOR
        public float gizmosSize = 5f;
        private void OnDrawGizmos()
        {
            Gizmos.color = Color.green;
            Gizmos.DrawSphere(StartObject.transform.position, gizmosSize);

            Gizmos.color = Color.red;
            Gizmos.DrawSphere(EndObject.transform.position, gizmosSize);
        }
#endif
        ...
        private void UpdateLineRenderer()
        {
            ...
            var array = new Vector2[segmentCount];
            ...
            array[index++] = (segments[startIndex].Key) - (Vector3)(GetComponent<RectTransform>().rect.size / 2f);


            for (int i = startIndex; i < segments.Count; i++)
            {
                //lineRenderer.SetPosition(index++, segments[i].Value);
                //array[index++] = segments[i].Value;

                array[index++] = (segments[i].Key) - (Vector3)(GetComponent<RectTransform>().rect.size / 2f);
            }
